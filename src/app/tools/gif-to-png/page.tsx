...
export default function GifToPngPage() {
  ...
  const handleFiles = useCallback((newFiles: FileList | null) => {
    ...
    const valid = Array.from(newFiles).filter((f) =>
      /\.(gif)$/i.test(f.name) && f.size <= MAX_FILE_SIZE
    );
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">GIF to PNG Converter</h1>
      ...
      <section className="space-y-4 py-8 border-t">
        <Tabs defaultValue="what">
          <TabsList>
            <TabsTrigger value="what">What is GIF to PNG?</TabsTrigger>
          ...
          <TabsContent value="what" className="mt-4 space-y-3 text-muted-foreground text-sm">
            <h2 className="text-lg font-semibold text-foreground">What is GIF to PNG Conversion?</h2>
            <p>
              GIF is an image format that supports animation and a limited 256-color palette. PNG supports millions
              of colors with lossless compression and alpha transparency. Converting GIF to PNG gives you higher color
              depth and better quality for static images.
            </p>
            <h3 className="text-base font-semibold text-foreground">When to Convert GIF to PNG</h3>
            <p>
              Convert GIF to PNG when you need a static thumbnail from an animated GIF, want better color accuracy
              than GIF&apos;s 256-color limit, or need smooth alpha transparency instead of GIF&apos;s binary transparency
              (pixel is either fully transparent or fully opaque).
            </p>
          </TabsContent>
          ...
          <TabsContent value="how" className="mt-4 space-y-3 text-muted-foreground text-sm">
            ...
            <ol className="list-decimal list-inside space-y-2">
              <li>Drag and drop your GIF files onto the upload area, or click to browse.</li>
              <li>You can add multiple files at once for batch conversion.</li>
              <li>Click &quot;Convert to PNG&quot; to start the conversion.</li>
              <li>For animated GIFs, the first frame is extracted as a static PNG.</li>
          ...
          <TabsContent value="faq" className="mt-4 space-y-4 text-muted-foreground text-sm">
            ...
            <div>
              <h3 className="text-sm font-semibold text-foreground">What happens with animated GIFs?</h3>
              <p>Animated GIFs are converted by extracting the first frame as a static PNG image. If you need to convert an animated GIF to video, use our GIF to MP4 tool instead.</p>
            </div>
            <div>
              <h3 className="text-sm font-semibold text-foreground">Will the PNG be larger than the GIF?</h3>
              <p>For static GIFs, the PNG may be slightly larger or smaller depending on the image. For animated GIFs, the resulting PNG (single frame) will be much smaller since it only contains one frame.</p>
            </div>
            <div>
              <h3 className="text-sm font-semibold text-foreground">Does PNG support transparency like GIF?</h3>
              <p>Yes, and PNG does it better. GIF only supports binary transparency (fully transparent or fully opaque), while PNG supports smooth alpha transparency with 256 levels of opacity.</p>
            </div>
          </TabsContent>
        </Tabs>
      </section>
